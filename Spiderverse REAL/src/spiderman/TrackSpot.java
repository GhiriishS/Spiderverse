...
public class TrackSpot {
    ...
    public static void main(String[] args) {
        ...
        StdIn.readLine();
        int targetDimension = StdIn.readInt();
        ...
        ArrayList<Integer> dimensionsVisited = new ArrayList<Integer>();

        findSpot(adjacencyList, startDimension, targetDimension, dimensionsVisited);
        dimensionsVisited.add(targetDimension);

        StdOut.setFile(outputFile);
        for (int i = 0; i < dimensionsVisited.size(); i++) {
            if (i == dimensionsVisited.size() - 1) {
                StdOut.print(dimensionsVisited.get(i));
            }
            else {
                StdOut.print(dimensionsVisited.get(i) + " ");
            }
    ...
    public static ArrayList<Integer> findSpot(HashMap<Integer, ArrayList<Integer>> adjacencyList,
            int currentDimension, int targetDimension, ArrayList<Integer> dimensionsVisited) {
        
                if (currentDimension == targetDimension) {
            ArrayList<Integer> pathToDestination = new ArrayList<>();
            pathToDestination.add(currentDimension);
            return pathToDestination;
        }

        dimensionsVisited.add(currentDimension);

        ArrayList<Integer> adjacentDimensions = adjacencyList.get(currentDimension);

        if (adjacentDimensions != null) {
            for (Integer dimension : adjacentDimensions) {
                if (!dimensionsVisited.contains(dimension)) {
                    ArrayList<Integer> pathToDestination = findSpot(adjacencyList, dimension, targetDimension,
                            dimensionsVisited);
                    if (pathToDestination != null) {
                        pathToDestination.add(0, currentDimension);
                        return pathToDestination;
                    }
                }
            }
